...
| **`make eval` exits 2 — N<50** | Positive class is no longer empty: the live-API probe contributes **2 real violations, both detected**. Scored pool 8 (6 controls + 2 probed bounds), plus 8 unlabelled merchant profiles. The five real drifts sit in the discovery set and are excluded by design to avoid inflating the rate. **Closing this is research labour, not configuration**: ~50 independently-sourced claims, hand-labelled against circulars the way OC-201 and OC-228 were. It is the largest remaining gap in the project — larger than the payment integration. (`FAILURES.md` #5) |
| **Drift #2 corrected** | We claimed Razorpay's MCP server had *"no limit validation at all"*. **False** — `max_amount` is mandatory and both bounds are enforced server-side. The accurate claim is narrower: the tool schema does not *document* them, so a caller learns a bound by having a request refused. `FAILURES.md` #6. |
...
| Kill-gate 2 — **PASSED 2026-08-27** | `create_order` with `token.type="single_block_multiple_debit"` returns **HTTP 200** against live test keys. The primary rail works; `make demo` now creates **real Razorpay orders**. Note: Subscriptions/Plans return **401** on this account, so the documented fallback (UPI Autopay) was **not** available — had the gate failed, so would the fallback. |
| Live API vs circular — **two new drifts** | Probed by binary search: the API accepts `max_amount` up to **₹15,000** (OC-228 §5 authorises **₹10,000**) and `expire_at` up to **91 days** (circular says 90; the API's own error text says 90 too). See `research/11_final_selection/LIVE_API_FINDINGS.md` — including why this is *not* stated as "Razorpay is wrong". |
## Components
